...
class HealthResponse(BaseModel):
    ...
    database_connected: bool = Field(True, description="Status indicating if the database is reachable", json_schema_extra={"example": True})
    model_version: str = Field(..., description="Loaded model version string", json_schema_extra={"example": "0.1.0"})
    uptime_seconds: Optional[float] = Field(None, description="Application uptime in seconds", json_schema_extra={"example": 142.5})
